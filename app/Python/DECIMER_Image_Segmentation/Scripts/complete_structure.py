...
def set_x_diff_range(x_diff: float, eukl_distance: float, image_array: np.array) -> np.array:
    ...
    blur_factor = int(
        image_array.shape[1]/500) if image_array.shape[1]/500 >= 1 else 1
    if x_diff > 0:
    ...
    for node_index in range(len(bounding_box)):
        ...
        if x_diff == 0:
            bounding_box = adapt_x_values(
                bounding_box=bounding_box, node_index=node_index, image_shape=image_array.shape)
        # Define amount of steps we have to go in x-direction to get from node 1 to node 2
        x_diff = bounding_box[node_index][0] - bounding_box[node_index-1][0]
        x_diff_range = set_x_diff_range(x_diff, euklidian_distance(
            bounding_box[node_index], bounding_box[node_index-1]), image_array)
...
def expand_masks(image_array: np.array, seed_pixels: List[Tuple[int, int]], mask_array: np.array, contour_expansion=False) -> np.array:
    ...
    for seed_pixel in seed_pixels:
        neighbour_pixels = determine_neighbour_pixels(
            seed_pixel, image_array.shape)
...
def expansion_coordination(mask_array: np.array, image_array: np.array) -> np.array:
    ...
    if seed_pixels != []:
        ...
        seed_pixels = find_seeds_contours(
...
def complete_structure_mask(image_array: np.array, mask_array: np.array, debug=False) -> np.array:
    """This function takes an image (array) and an array containing the masks (shape: x,y,n where n is the amount of masks and x and y are the pixel coordinates).
    ...
    if mask_array.size != 0:
        ...
        blur_factor = int(
            image_array.shape[1]/185) if image_array.shape[1]/185 >= 2 else 2
        ...
        blurred_image_array = binary_erosion(
            binarized_image_array, selem=kernel)
